...
export const useUpdater = () => {
  ...
  const logToBackend = useCallback(
    (eventType: string, message: string, latestVersion: string | null = null, details?: string) => {
  ...
  const checkForUpdates = useCallback(
    async (manual = false) => {
      try {
        ...
        addLog(
          'Checking updater endpoint: https://github.com/justinlevinedotme/FMMLoader-26/releases/latest/download/latest.json'
        );
        ...
        addLog(
          `Update found! Current version: ${update.currentVersion}, Latest version: ${update.version}`
        );
    ...
    try {
      ...
      await update.downloadAndInstall((event) => {
        switch (event.event) {
          case 'Started':
            addLog(
              `Download started - Content length: ${event.data.contentLength || 'unknown'} bytes`
            );
      ...
      logToBackend(
        'ERROR',
        `Error downloading/installing update: ${errorMessage}`,
